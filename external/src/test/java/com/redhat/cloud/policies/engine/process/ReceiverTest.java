...
import com.redhat.cloud.policies.engine.history.AlertsHistoryEntry;
...
//        evCond.setExpression("facts.arch = 'string'");
    ...
    public void testReceiver() throws Exception {
        ...
        checkAlertsHistoryEntries(fullTrigger, 1);
        checkAlertsHistoryEntries(fullTrigger2, 1);
        ...
        checkAlertsHistoryEntries(fullTrigger, 2);
        checkAlertsHistoryEntries(fullTrigger2, 2);
    ...
    private void clearAlertsHistory() {
        Transaction transaction = session.beginTransaction();
        session.createQuery("DELETE FROM AlertsHistoryEntry").executeUpdate();
        transaction.commit();
    }

    private void checkAlertsHistoryEntries(FullTrigger trigger, int expectedSize) {
        String query = "SELECT e FROM AlertsHistoryEntry e WHERE e.tenantId = :tenantId AND e.policyId = :policyId";
        List<AlertsHistoryEntry> history = session.createQuery(query, AlertsHistoryEntry.class)
